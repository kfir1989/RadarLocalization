...
def ndt_match(src_points, dst_points, resolution=7, max_iter=20, tol=1e-5):
    ...
    def objective_func_vectorized(x):
        ...
        d = np.abs(valid_transformed_points - valid_cells['mean'])
        cov_inv = np.linalg.pinv(valid_cells['cov'])
        cost = np.abs(np.sum(np.einsum('...ij,...j->...i', d @ cov_inv, d)))

        cost += (transformed_points.shape[0]-valid_transformed_points.shape[0]) * 1e9
        #print(f"cost = {cost}")
        return cost
