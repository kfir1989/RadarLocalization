...
class NuscenesSimulation():
    def __init__(self, nusc, model, scene_id, **kwargs):
        ...
        base_dir = kwargs.pop('base_dir', '/home/kfir/workspace/RadarLocalization/images')
        dname = kwargs.pop('dname', 'database')
        self.nmax = kwargs.pop('Nmax', 1800)
        method = kwargs.pop('method', '')
        self.mm = model.mm
        self.dataset = NuscenesDataset(nusc=nusc, directory=directory, scene_id=scene_id, N=self.nmax, exp_params=self.exp_params)
        self.video = NuscenesVideo(history=True, scene=scene_id)
        self.video_debug = NuscenesVideoDebug(history=True, scene=scene_id)
        self.video_pf = PFVideo(history=True, scene=scene_id, N=self.nmax, method=method)
        ...
        self.database = NuscenesProcessedDatabase(scene_id=scene_id, base_dir=base_dir, dname=dname)
        self.lane = None
        self.scene = scene_id
    ...
    def run(self,start, N, generate_video=False, video_with_priors=False, debug=False, translate=True, exp_params=None):
        start_idx = start
        first = True
        gt_pos = np.zeros([N,2])
        pf_pos = np.zeros([N,2])
        for t in tqdm(range(start_idx,start_idx + N)):
            print(f"frame {t}")
            #get data
            zw, covw, prior, dw, video_data, nusc_map = self.dataset.getData(t)
            translation = np.array(video_data["ego_path"][0])[0:2]
            #run model
            points, polynoms, dynamic_tracks, dynamic_clusters, debug_info, mm_results = self.model.run({"zw":zw, "covw":covw}, {"dw":dw}, video_data, prior, translation, nusc_map, exp_params)
            #Draw plots
